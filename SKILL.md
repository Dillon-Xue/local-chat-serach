...
## 已知限制 / 后续迭代
- 语义搜索（理解意图而非精确词）**不实现**：纯关键词引擎已覆盖 FR-01/03/04/05/06/07/08/09/10/11。本地 embedding 模型在 WorkBuddy 运行环境（Windows 侧，无独立 Python 推理栈）中难以落地，故 FR-02 关闭，不做 v2。
- 极长的「所有项目」检索为流式逐文件扫描 + temp 缓存索引，稳态通常 <3s。
